import mysql from 'mysql2'; // 导入 mysql2 包

import config from './config.json' assert { type: 'json' }; // 导入数据库连接配置信息
...
const query = (sql, params) => {
  return new Promise(async (resolve, reject) => {
    const connection = await connect(); // 获取数据库连接
